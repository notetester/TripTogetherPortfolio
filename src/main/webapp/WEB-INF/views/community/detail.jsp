                        ...
                        <c:if test="${isAdminMode and not isOwner}">
                            <c:choose>
                                <c:when test="${post.accountStatus eq 'BLOCKED'}">
                                    <button class="block-btn unblock" onclick="unblockUser(${post.userIdx})">👤 유저차단해제
                                    </button>
                                </c:when>
                                <c:otherwise>
                            ...
                            <c:choose>
                                <c:when test="${post.postStatus eq 'DORMANT'}">
                                    <button class="block-btn unblock" onclick="unblockPost(${post.postId})">🚫 글차단해제
                                    </button>
                                </c:when>
                                <c:otherwise>
                    ...
                    <div class="slider-track" id="sliderTrack">
                        <c:forEach var="img" items="${imageList}">
                            <div class="slide-item">
                                <img src="${pageContext.request.contextPath}${img.imageUrl}" alt="${post.title}"
                                     loading="lazy"
                    ...
                    <c:otherwise>
                        <c:forEach var="comment" items="${commentList}">
                            ...
                            <c:if test="${empty comment.parentCommentId}">
                                <c:choose>
                                    <%-- 차단된 댓글: 유저경험모드/일반유저는 blind --%>
                                    <c:when test="${(comment.commentStatus eq 'BLOCKED' or comment.accountStatus eq 'BLOCKED') and !isAdminMode}">
                                    </c:when>
                                    <%-- ACTIVE 또는 관리자모드 --%>
                                    <c:otherwise>
                                                            ...
                                                            <c:if test="${isAdminMode and sessionScope.loginUser.userIdx ne comment.userIdx}">
                                                                <c:choose>
                                                                    <c:when test="${comment.accountStatus eq 'BLOCKED'}">
                                                                        <button class="block-btn unblock"
                                                                                onclick="unblockUser(${comment.userIdx})">
                                                                            👤 유저차단해제
                                                                        </button>
                                                                    </c:when>
                                                                    <c:otherwise>
                                                                        <button class="block-btn"
                                                                                onclick="blockUser(${comment.userIdx})">
                                                                            👤 유저차단
                                                                        </button>
                                                                    </c:otherwise>
                                                                </c:choose>
                                                                <c:choose>
                                                                    <c:when test="${comment.commentStatus eq 'BLOCKED'}">
                                                                        <button class="block-btn unblock"
                                                                                onclick="unblockComment(${comment.commentId})">
                                                                            🚫 댓글차단해제
                                                                        </button>
                                                                    </c:when>
                                                                    <c:otherwise>
                                                                        <button class="block-btn"
                                                                                onclick="blockComment(${comment.commentId})">
                                                                            🚫 댓글차단
                                                                        </button>
                                                                    </c:otherwise>
                                                                </c:choose>
                                ...
                                <fmt:formatDate value="${comment.createdAt}" pattern="yyyy-MM-dd"/>
                                                            ...
                                                            <c:if test="${isOwner and post.postType eq 'question' and not isSolved and comment.commentId ne acceptedCommentId}">
                                                                <button class="accept-btn"
                                                                        onclick="acceptComment(${post.postId}, ${comment.commentId})">
                                                                    채택하기
                                                                </button>
                                                            </c:if>
                                                            ...
                                                            <c:choose>
                                                                <c:when test="${not empty sessionScope.loginUser}">
                                                                    <button class="comment-like-btn"
                                                                        ...
                                                                        &#10084; <span
                                                                            id="commentLikeCount_${comment.commentId}">${comment.likeCount}</span>
                                                                    </button>
                                ...
                                <textarea class="reply-textarea" id="replyText_${comment.commentId}"
                                          ...
                                          onkeydown="if(event.key==='Enter' && !event.shiftKey){event.preventDefault(); submitReply(${post.postId}, ${comment.commentId});}"></textarea>
                                                                <div class="reply-input-actions">
                                                                    ...
                                                                    <button class="reply-submit-btn"
                                                                            onclick="submitReply(${post.postId}, ${comment.commentId})">
                                                                        등록
                                                                    </button>
                                                                </div>
                                                            </div>
                                                    ...
                                                    <c:if test="${isAdminMode and (comment.commentStatus eq 'BLOCKED' or comment.accountStatus eq 'BLOCKED')}">
                                                        <button class="post-admin-delete-btn"
                                                                onclick="adminDeleteComment(event, ${comment.commentId})">
                                                            ✕
                                                        </button>
                                                    </c:if>
                                                </div>
                                            ...
                                            <c:forEach var="reply" items="${commentList}">
                                                <c:if test="${reply.parentCommentId eq comment.commentId}">
                                                    <c:choose>
                                                        <%-- 차단된 대댓글: 유저경험모드/일반유저는 blind --%>
                                                        <c:when test="${(reply.commentStatus eq 'BLOCKED' or reply.accountStatus eq 'BLOCKED') and !isAdminMode}">
                                                        </c:when>
                                                        <c:otherwise>
                                                                            ...
                                                                            <div class="comment-top">
                                                                                ...
                                                                                <c:if test="${isAdminMode and sessionScope.loginUser.userIdx ne reply.userIdx}">
                                                                                    <c:choose>
                                                                                        <c:when test="${reply.accountStatus eq 'BLOCKED'}">
                                                                                            <button class="block-btn unblock"
                                                                                                    onclick="unblockUser(${reply.userIdx})">
                                                                                                👤 유저차단해제
                                                                                            </button>
                                                                                        </c:when>
                                                                                        <c:otherwise>
                                                                                            <button class="block-btn"
                                                                                                    onclick="blockUser(${reply.userIdx})">
                                                                                                👤 유저차단
                                                                                            </button>
                                                                                        </c:otherwise>
                                                                                    </c:choose>
                                                                                    <c:choose>
                                                                                        <c:when test="${reply.commentStatus eq 'BLOCKED'}">
                                                                                            <button class="block-btn unblock"
                                                                                                    onclick="unblockComment(${reply.commentId})">
                                                                                                🚫 댓글차단해제
                                                                                            </button>
                                                                                        </c:when>
                                                                                        <c:otherwise>
                                                                                            <button class="block-btn"
                                                                                                    onclick="blockComment(${reply.commentId})">
                                                                                                🚫 댓글차단
                                                                                            </button>
                                                                                        </c:otherwise>
                                                                                    </c:choose>
                                          ...
                                          <fmt:formatDate value="${reply.createdAt}" pattern="yyyy-MM-dd"/>
                                                                                ...
                                                                                <c:if test="${not empty sessionScope.loginUser and sessionScope.loginUser.userIdx ne reply.userIdx and not isAdminMode}">
                                                                                    <button class="report-btn"
                                                                                            onclick="reportComment(${reply.commentId})">
                                                                                        &#9888; 신고
                                                                                    </button>
                                                                                </c:if>
                                                                                ...
                                                                                <c:choose>
                                                                                    <c:when test="${not empty sessionScope.loginUser}">
                                                                                        <button class="comment-like-btn"
                                                                                            ...
                                                                                            &#10084; <span
                                                                                                id="commentLikeCount_${reply.commentId}">${reply.likeCount}</span>
                                                                                        </button>
                                                                                    </c:when>
                                                                                    <c:otherwise>
                                                                                        <button class="comment-like-btn"
                                                                        ...
                                                                        <c:if test="${isAdminMode and (reply.commentStatus eq 'BLOCKED' or reply.accountStatus eq 'BLOCKED')}">
                                                                            <button class="post-admin-delete-btn"
                                                                                    onclick="adminDeleteComment(event, ${reply.commentId})">
                                                                                ✕
                                                                            </button>
                                                                        </c:if>
                                                                    </div>
              ...
              <textarea class="comment-textarea" id="commentText"
                    ...
                    <c:otherwise>
                        <div class="comment-login-notice">
                            <p>댓글을 작성하려면
                                <a href="${pageContext.request.contextPath}/auth/login" class="login-link">로그인</a>이
                                필요합니다.
                            </p>
                        </div>
                    </c:otherwise>
                    ...
                    <div class="related-card"
                        ...
                        <c:choose>
                            <c:when test="${not empty r.thumbUrl}">
                                <img class="related-img" src="${pageContext.request.contextPath}${r.thumbUrl}"
                                     alt="${r.title}" loading="lazy">
                            </c:when>
                            <c:otherwise>
                                <div class="related-img"
                                     style="background:var(--gray-100);display:flex;align-items:center;justify-content:center;font-size:36px;">
                                    ✈️
                                </div>
                            </c:otherwise>
                        </c:choose>
...
<script>
    ...
    function blockUser(userIdx) {
        ...
        fetch(CTX + '/community/user/' + userIdx + '/block', {
            method: 'POST',
            headers: {'X-Requested-With': 'XMLHttpRequest'}
        })
            .then(function (res) {
    ...
    function unblockUser(userIdx) {
        ...
        fetch(CTX + '/community/user/' + userIdx + '/unblock', {
            method: 'POST',
            headers: {'X-Requested-With': 'XMLHttpRequest'}
        })
            .then(function (res) {
    ...
    function blockPost(postId) {
        ...
        fetch(CTX + '/community/' + postId + '/block', {
            method: 'POST',
            headers: {'X-Requested-With': 'XMLHttpRequest'}
        })
            .then(function (res) {
    ...
    function unblockPost(postId) {
        ...
        fetch(CTX + '/community/' + postId + '/unblock', {
            method: 'POST',
            headers: {'X-Requested-With': 'XMLHttpRequest'}
        })
            .then(function (res) {
    ...
    function blockComment(commentId) {
        ...
        fetch(CTX + '/community/comment/' + commentId + '/block', {
            method: 'POST',
            headers: {'X-Requested-With': 'XMLHttpRequest'}
        })
            .then(function (res) {
    ...
    function unblockComment(commentId) {
        ...
        fetch(CTX + '/community/comment/' + commentId + '/unblock', {
            method: 'POST',
            headers: {'X-Requested-With': 'XMLHttpRequest'}
        })
            .then(function (res) {

...
public class HttpSecurityAssessmentProvider implements LoginRiskAssessmentProvider {
    ...
    public Optional<LoginRiskAssessmentResult> assess(LoginRiskAssessmentRequest request) {
        return loginRiskPolicyMapper.findEnabledExternalAssessmentProviders().stream()
                .filter(provider -> supports(provider.getProviderKind()))
                .filter(provider -> provider.getEndpointUrl() != null && !provider.getEndpointUrl().isBlank())
                .map(provider -> callProvider(provider, request))
                .filter(Optional::isPresent)
                .map(Optional::get)
                .findFirst();
    }

    private boolean supports(String providerKind) {
        return "AI_MODEL".equals(providerKind) || "POLICY_AUTHORITY".equals(providerKind);
    }

    private Optional<LoginRiskAssessmentResult> callProvider(SecurityAssessmentProviderConfigVO provider,
        ...
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                ...
                return failClosedResult(provider, request, "HTTP " + response.statusCode());
            }
            ...
            return failClosedResult(provider, request, e.getMessage());
        }
    }

...
- `assistant` — Multi-turn AI travel assistant via Codex API (`Codex-3-5-haiku-20241022`). Conversation history managed in-memory per request.
- `common` — `MainController` (home/index routing) and `ChatbotController` (site navigation chatbot via `Codex-sonnet-4-20250514` that returns structured JSON with links and quick replies).
...
- **AssistantServiceImpl**: Multi-turn chat with `Codex-3-5-haiku-20241022`. Keeps up to 20 history messages. Requires `Codex.api.key` in properties.
- **ChatbotService**: Site-navigation chatbot with `Codex-sonnet-4-20250514`. Returns structured JSON (`message`, `links[]`, `quickReplies[]`). Parses JSON from Codex's response and strips code fences before parsing.
...
- 사용자는 **merge 방식 선호** — rebase는 사용자 명시 요청 시에만 사용
- push 진행 전 반드시 다음 순서:
  1. `git fetch origin` 실행 (원격 정보 갱신, 로컬 변경 없음)
  2. `git log --oneline Victor..origin/dev` 로 dev에 새 커밋이 있는지 확인
  3. dev에 새 커밋이 있으면 다음 사전 분석을 사용자에게 보고:
     - dev의 변경 파일 리스트 (`git diff --name-only Victor...origin/dev`)
     - Victor가 건드린 파일과 비교해 잠재적 충돌 가능성
  4. 사용자가 진행 방향 결정:
     - **그대로 push** (Victor만 푸시) — 통합은 PR 시점에 GitHub에서
     - **merge 후 push** — `git merge origin/dev` 시도 (사용자 선호)
  5. merge 진행 중 conflict 발생 시 Codex가 해결 지원:
     - `git status`로 충돌 파일 목록 확보
     - 각 충돌 파일을 Read로 분석 (충돌 마커 양쪽 의미 파악)
     - 사용자에게 보고: "X 파일 N줄: dev는 A, Victor는 B. 어느 쪽 우선 / 어떻게 통합?"
     - 사용자 결정 후 Edit로 충돌 마커 제거 + 통합 코드 작성
